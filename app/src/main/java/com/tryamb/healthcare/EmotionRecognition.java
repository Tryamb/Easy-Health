...
import com.tryamb.healthcare.classifiers.TFLiteImageClassifier;
import com.tryamb.healthcare.utils.ImageUtils;
import com.tryamb.healthcare.utils.SortingHelper;
...
public class EmotionRecognition extends AppCompatActivity {
    ...
    private ImageView mPickImageButton;
...
//        if (reversedKeys.size() > 5) {
//            reversedKeys = new ArrayList<>(reversedKeys.subList(0, 5));
//        }
        ...
        double positiveEmotions=0.001, negativeEmotions=0.001;
        boolean method2=false;
        String finalResult="";
        for (Pair<String, String> pair : faceGroup) {
            String key = pair.first;
            String percentageStr = pair.second;
            double percentage = Double.parseDouble(percentageStr.replace("%", ""));
            if (key.equals("sad") && percentage>20) {
                method2=true;
            }
                if (key.equals("happy") || key.equals("surprise")) {
                    positiveEmotions += percentage;
                } else {
                    negativeEmotions += percentage;
                }
        }

        if(method2){
            if(negativeEmotions-positiveEmotions>35) {
                finalResult = "You are currently in a state of profound sorrow and deep loneliness, characterized by intense emotions.";
            }
                else if(negativeEmotions-positiveEmotions>25){
                finalResult = "Looks like You are experiencing moderate stress due to extent of sad expression on your face.";

            }
                else{
                finalResult = "You are sad because something is not happening according to plan.";
            }
        }
        else {
            double resultant = positiveEmotions / negativeEmotions;
            if (resultant > 10) {
                finalResult += "You are Happy- an emotion of joy, gladness, satisfaction, and well-being";
            } else if (resultant >= 1) {
                finalResult += "You are Okay";
            } else if (resultant >= 0.1) {
                finalResult += "You are almost okay";
            } else if (resultant >= 0.025) {
                finalResult += "It's like you are feeling a little bit bothered or distracted.";
            } else if (resultant >= 0.05) {
                finalResult += "You're a bit worried, thinking about something. It shows on your face with a sad look.";
            } else if (resultant >= 0.01) {
                finalResult += "Looks like You are sad and mild tensed.";
            }
            else if (resultant >= 0.005) {
                finalResult += "Looks like You are experiencing moderate stress due to extent of sad expression on your face.";
            } else {
                finalResult += "You're experiencing a sense of loneliness and feeling lost in life, which is causing a deep depression.";
            }
        }



        faceGroup.add(new Pair<>("Final Result", finalResult));
