...
public class ModelInference {
    ...
    public void segment_and_classify(float[][][][] input1, float[][][][] input2, float[][][][] input3, FirebaseModelInterpreterOptions options2, final FirebaseCustomLocalModel localModel_effnet, TimeoutCallback timeoutCallback) {
        try {
                    ...
                    .addOnSuccessListener(
                            new OnSuccessListener<FirebaseModelOutputs>() {
                                ...
                                public void onSuccess(FirebaseModelOutputs result) {
                                    Log.e("done_masking", "done masking");
                                    Log.e("save", "THis is save: "+save);
    ...
    private void classify(FirebaseModelInterpreter effnet_interpreter, FirebaseModelInputOutputOptions inputOutputOptions) {
        ...
        SkinScanActivity.bmpOutputs = preprocessor.initBmpOutputs(contours);
        bmps = preprocessor.initBmps(contours);
        SkinScanActivity.croppedBitmaps.removeAll(SkinScanActivity.croppedBitmaps);
    ...
    private void set_output_parameters(FirebaseModelOutputs result, Point pnt, RectangleRange rectangleRange, Mat orig_changed, List<MatOfPoint> contour, Rect finalRect, boolean b) {
        SkinScanActivity.bmpOutputs[counter] = result.getOutput(0);

        Character numb = (char)('A'+counter);
        EfficientOuput efficientOuput = new EfficientOuput();
        String a = efficientOuput.outputToString(SkinScanActivity.bmpOutputs[counter]);
        ...
        iv = ((SkinScanActivity) context).findViewById(R.id.image_view);
        select_btn = ((SkinScanActivity) context).findViewById(R.id.imageView);
        ...
        mainActivity = (SkinScanActivity) context;
